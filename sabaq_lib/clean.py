...
if __name__ == "__main__":
    ...
    #       important in idioms
    idioms = map(lambda doc: [x.lemma_ for x in doc if not x.is_punct], idioms)

    # Map to a list of examples
    exs = map(lambda x: x["examples"], db["fr_wiktionary"].find({}))
    ...
    exs = list(map(lambda doc: tuple([x.lemma_ for x in doc if not x.is_punct]), exs))
